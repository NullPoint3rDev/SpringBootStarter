...
public class PropertyGuardEnvironmentPostProcessor implements EnvironmentPostProcessor {
    ...
    private void check(String defaultExecutor, String cronExpression, String timeValue, String taskSize) {
        ...
        if (isInvalidType) {
            throw new BackgroundTaskPropertyException("Property background-executor.default-executor must be" +
                    " cron or time!");
        }
        ...
        if (Objects.equals(defaultExecutor, "time") && !StringUtils.hasText(timeValue)) {
            throw new BackgroundTaskPropertyException("Invalid time expression for 'time' type!");
        }

        if (!StringUtils.hasText(taskSize) || !taskSize.matches("-?\\d*") || Integer.parseInt(taskSize)
                <= 0) {
